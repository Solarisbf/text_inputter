...
static int previousKey = 0; // for keyCount
static int INS = 10;  //   10 : INSERT MODE    , 20 : NORMAL MODE      //잘보면 기본이 인서트 모드임. 노말모드를 만들어야함.
...
int main(int argc, char **argv){
 ...
 if (( dev = open(fnd_dev,O_RDWR | O_SYNC ) ) < 0 ) {
...
// 물리어드레스를 가상 어드레스로 변환
   pTextFnd = mmap(NULL,1,PROT_WRITE, MAP_SHARED, keyFd,ADDRESSOFFND);//fnd_mmap
...
void inputKey() {
 ...
 while(1){
	...
	if ( c != previousKey) {
	   ...
	   int p = previousKey;
		if ( /*p!=1 && p!=2 && p!=3 && p!=4 && p!=8 &&*/ c!=1 && c!=2 
		     && c!=3 && c!=4 && c!=8) { 
			WDCOUNT++;
		if (p!=1 && p!=2 && p!=3 && p!=4 && p!=8 && WDCOUNT != 1) {
			setcommand(0x14);
		}
	}
      }
   ...
   case KEY_NUM2:  writebyte(' ');
		  
	       setcommand(0x10);    // DEL    // 글자가 있나없나 검사하고, WDCOUNT-- 해야함.
		   //setcommand(0x18);    // 이거 스크린 <- 시프트임 . 틀림.
		   WDCOUNT--;
		   break;
   ...
   case KEY_NUM4: 
      if(CAPS == 0) {
       ...
       break;  // CAPS
   case KEY_NUM5:   // WDCOUNT++; must not be here................
		   keyCount++;
       if( keyCount % 3 == 1) 
        writebyte('.');   //.
       else if( keyCount % 3 == 2) 
